    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth scroll="paper">
      ...
      <DialogContent dividers>
        ...
        <Section title="Views">
          ...
          <P>
            <strong>Off-Role Picks</strong> — Champions with the highest floor win rate for minimal
            investment, drawn from slope iterations joined with Games to 50% data. Default sort is
            by Floor WR (initial_wr) descending — the win rate observed in the 5–25 games mastery
            bracket. Columns: Champion, Lane, Pickup tier (slope_tier), Floor WR, Est. Games to
            50%, and Peak WR. Use this view to identify champions that perform well from the very
            first games without requiring deep mastery investment.
          </P>
          <P>
            <strong>Learning Profile</strong> — A scatter chart answering two questions: how long
            does a champion take to plateau (Q4), and how much win rate is available to gain through
            mastery (Q5)? The X axis shows inflection_games — the estimated number of games until
            the champion's win rate flattens near its peak. The Y axis shows peak WR minus initial
            WR in percentage points (pp), representing the total win rate gain achievable from
            mastery investment. Quadrant reference lines are drawn at the median inflection_games
            (vertical) and at 3 pp WR gain (horizontal), dividing champions into four learning
            archetypes.
          </P>
        </Section>
        ...
        <Section title="Scoring Methodology">
          ...
          <P>
            The estimated number of games at which a champion's learning curve flattens — i.e.,
            where the slope drops below a threshold indicating near-peak performance. Derived from
            the slope iterations analysis as{' '}
            <code>inflection_mastery / 700</code>, where inflection mastery is the first bracket
            entry point where smoothed win rate reaches within 0.5 pp of peak. Used as the X axis
            of the Learning Profile chart.
          </P>

          <Typography variant="subtitle2" gutterBottom fontWeight="bold" sx={{ mt: 2 }}>
            peak_wr − initial_wr (WR Gain, pp)
          </Typography>
          <P>
            The Y axis of the Learning Profile chart. Represents the maximum achievable improvement
            in win rate from a player's first games (the 5–25 games mastery bracket, captured as
            initial_wr) to peak performance (peak_wr). Expressed in percentage points (pp). A high
            value means significant win rate is gated behind mastery investment; a low value means
            the champion performs near its ceiling from the start.
          </P>
        </Section>
